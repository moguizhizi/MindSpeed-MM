...
from mindspeed_rl.trainer.orm_trainer import ORMTrainer
...
def rm_train(args):
    ...
    model, optimizer, opt_param_scheduler = setup_model_and_optimizer(rm_model_provider, model_type)
    ...
    parent_path = Path(args.data_path[0]).parent
    bin_files = list(parent_path.glob("*.bin"))
    if len(bin_files) > 0:
        # build dataset
    ...
    if train_dataset is not None and len(train_dataset) > 0:
        train_dataloader = PreferenceDataLoader(
    ...
    if valid_dataset is not None and len(valid_dataset) > 0:
        valid_dataloader = PreferenceDataLoader(
    ...
    if test_dataset is not None and len(test_dataset) > 0:
        test_dataloader = PreferenceDataLoader(
...
def separate_config_and_parse_args(config):
    ...
    sft_config_dict = OmegaConf.to_container(reward_config, resolve=True)
...
@hydra.main(config_path='../configs', config_name='orm_trainer_qwen25_7b', version_base=None)
def main(config):
